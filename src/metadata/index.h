...
namespace cache {
  class Index {
    ...
    protected:
      ...
      uint32_t _n_buckets, _n_elems_per_bucket;
  };

  class LBAIndex : Index {
    public:
      LBAIndex(uint32_t n_bits_per_key, uint32_t n_bits_per_value,
          uint32_t n_buckets, uint32_t n_elems_per_bucket, std::shared_ptr<CAIndex> ca_index);
  ...
  class CAIndex : Index {
    ...
    private:
      uint32_t compute_ssd_loaction(uint32_t bucket_no, uint32_t index);
