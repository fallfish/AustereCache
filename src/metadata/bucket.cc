...
namespace cache {
  ...
  void LBABucket::update(uint32_t lba_sig, uint32_t ca_hash, std::shared_ptr<CAIndex> ca_index) {
    uint32_t v = 0;
    uint32_t slot_id = lookup(lba_sig, v);
    if (slot_id != ~((uint32_t)0)) {
      if (v != ca_hash) {
        set_v(slot_id, ca_hash);
      }
    } else {
      slot_id = _cache_policy.allocate(std::move(ca_slot_id));
  ...
  {
    uint32_t slot_id = 0, space = 0;
    while (slot_id < _n_slots) {
      if (is_valid(slot_id)) {
        if (get_k(slot_id) == ca_sig) {
          size = get_space(slot_id);
          return slot_id;
        }
        slot_id += space;
      }
