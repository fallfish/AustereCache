...
namespace cache {
  ...
  Index::Index(uint32_t n_bits_per_key, uint32_t n_bits_per_value,
      uint32_t n_buckets, uint32_t n_elems_per_bucket) :
    _n_bits_per_key(n_bits_per_key), _n_bits_per_value(n_bits_per_value),
    _n_buckets(n_buckets), _n_elems_per_bucket(n_elems_per_bucket)
  {};

  LBAIndex::LBAIndex(uint32_t n_bits_per_key, uint32_t n_bits_per_value,
      uint32_t n_buckets, uint32_t n_elems_per_bucket) :
    Index(n_bits_per_key, n_bits_per_value, n_buckets, n_elems_per_bucket)
  {
    _buckets.resize(n_buckets);
    for (int i = 0; i < n_buckets; i++) {
      _buckets[i] = std::move(
          std::make_unique<LBABucket>(
            n_bits_per_key, n_bits_per_value, n_elems_per_bucket)
          );
    }
  }
  ...
  bool LBAIndex::lookup(uint32_t lba_hash, uint32_t &ca_hash)
  {
    ...
    return _buckets[bucket_no]->find(signature, ca_hash) != -1;
  }
  ...
  uint32_t CAIndex::compute_ssd_location(uint32_t bucket_no, uint32_t index)
  {
    ...
    return (bucket_no * n_elems_per_bucket + index) *
  ...
  bool CAIndex::lookup(uint32_t ca_hash, uint32_t &size, uint32_t &ssd_location)
  {
    ...
    uint32_t value;
    uint32_t index = _buckets[bucket_no]->find(signature, value);
    clock_inc(value);
    _buckets[bucket_no]->set(index, value);
